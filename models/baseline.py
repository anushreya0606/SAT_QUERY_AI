...
class BaselineVLM:
    ...
    def predict(self, batch: Dict[str, Any]) -> Any:
        ...
        from models.domain_adapter import SensorPromptConditioner
        conditioner = SensorPromptConditioner()
        sensor = batch.get("sensor") or batch.get("sensor_domain") or batch.get("modality")
        raw_prompt = batch.get("prompt", "Describe this image.")
        if sensor:
            conditioned_prompt = conditioner.condition(raw_prompt, sensor=sensor, task_type=task_type)
        else:
            conditioned_prompt = raw_prompt

        if task_type in ("vqa", "cdvqa", "classification") and not any(k in conditioned_prompt.lower() for k in ["short answer", "single word", "choice"]):
            conditioned_prompt = f"{conditioned_prompt}\nAnswer with only the direct short answer or category label."

        full_prompt = self._LLAVA_TEMPLATE.format(prompt=conditioned_prompt)
    ...
    def _parse_output(self, raw_text: str, task_type: str) -> Any:
        ...
        if task_type in ("vqa", "cdvqa", "classification"):
            return self._clean_vqa_answer(raw_text)
        return raw_text.strip()

    def _clean_vqa_answer(self, text: str) -> str:
        """
        Clean and normalize raw text generated by the model into a concise VQA answer.
        Removes conversational filler, extracts choice letters/labels, and converts number words.
        """
        if not text:
            return ""
        ...
        preambles = [
            ...
            r"^in\s+this\s+image[,\s]*\s*",
            r"^i\s+can\s+see\s+(?:that\s+)?",
            r"^it\s+appears\s+to\s+be\s*",
        ...
        # If output starts with multiple choice format like "A) Forest" or "A. Forest" or "(A) Forest"
        choice_match = re.match(r"^\(?([a-dA-D])\)?[.:\s]+(.+)$", cleaned)
        if choice_match:
            cleaned = choice_match.group(2).strip()

        # Take first line if multiline response
        ...
        # Normalize standard number words
        num_map = {
            "zero": "0", "one": "1", "two": "2", "three": "3", "four": "4",
            "five": "5", "six": "6", "seven": "7", "eight": "8", "nine": "9", "ten": "10"
        }
        if cleaned.lower() in num_map:
            cleaned = num_map[cleaned.lower()]
